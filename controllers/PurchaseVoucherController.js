...
const getVouchers = async (req, res) => {
  try {
    ...
    const total = await PurchaseVoucher.countDocuments();

    // Fetch paginated invoices
    const voucher = await PurchaseVoucher.find().sort({voucherDate: 1}).skip(skip).limit(limit);
...
const addVoucher = async (req, res) => {
  try {
    const newVoucher = new PurchaseVoucher(req.body);
...
const updateVoucher = async (req, res) => {
  try {
    const updatedVoucher = await PurchaseVoucher.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true
    });
    if (!updatedVoucher) {
...
const deleteVoucher = async (req, res) => {
  try {
    const deletedVoucher = await PurchaseVoucher.findByIdAndDelete(req.params.id);
    if (!deletedVoucher) {
      return res.status(404).json({ message: "Voucher not found" });
    }
